...
(defcustom devcontainer-enable-port-forwarding nil
  "Enable automatic port forwarding from container to host.

When enabled, a background process monitors the container for
newly opened ports and automatically forwards them to the host.
This is useful for services like AWS SSO that use localhost callbacks."
  :group 'devcontainer
  :type 'boolean)
...
(defvar devcontainer--port-forwarder-process nil
  "The port forwarder background process for the current project.")

(defvar devcontainer--port-forwarder-alist nil
  "Alist mapping projects to their port forwarder processes.")
...
(defconst devcontainer--port-forwarder-script-content
  "#!/bin/bash
# port-forwarder.sh - Automatic port forwarding for devcontainers
# This script is generated by devcontainer.el
...
forward_port() {
    ...
    echo \"✓ Forwarding port $port (container -> host)\"

    # Use socat to forward from host to container
    socat TCP-LISTEN:\"$port\",fork,reuseaddr \\
          EXEC:\"docker exec -i $CONTAINER_ID socat STDIN TCP\\\\:localhost\\\\:$port\" \\
          &>/dev/null &
...
echo \"Starting port forwarder for container $CONTAINER_ID\"
echo \"Monitoring for new ports every 2 seconds...\"
echo \"\"
...
"
  "The content of the port forwarder script.")

(defun devcontainer--port-forwarder-script ()
  "Return the path to the port forwarder script.
If the script doesn't exist, create it from embedded content."
  (let* ((cache-dir (or (getenv "XDG_CACHE_HOME")
                        (expand-file-name "~/.cache")))
         (script-dir (expand-file-name "devcontainer.el" cache-dir))
         (script-path (expand-file-name "port-forwarder.sh" script-dir)))

    ;; Create directory if needed
    (unless (file-directory-p script-dir)
      (make-directory script-dir t))

    ;; Create script if it doesn't exist or is outdated
    (unless (and (file-exists-p script-path)
                 (equal (with-temp-buffer
                          (insert-file-contents script-path)
                          (buffer-string))
                        devcontainer--port-forwarder-script-content))
      (with-temp-file script-path
        (insert devcontainer--port-forwarder-script-content))
      (set-file-modes script-path #o755))

    script-path))

(defun devcontainer-start-port-forwarding ()
  "Start automatic port forwarding for the current devcontainer.

This starts a background process that monitors the container for
newly opened ports and automatically forwards them to the host."
  (interactive)
  (unless devcontainer-enable-port-forwarding
    (user-error "Port forwarding is not enabled.  Set `devcontainer-enable-port-forwarding' to t"))

  (when-let* ((project (project-current))
              (container-id (devcontainer-up-container-id))
              (script (devcontainer--port-forwarder-script)))

    ;; Stop existing forwarder for this project
    (devcontainer-stop-port-forwarding)

    (let* ((buffer-name (format "*devcontainer-port-forward: %s*"
      ...
      (setq devcontainer--port-forwarder-process proc)
      (push (cons project proc) devcontainer--port-forwarder-alist)

      (message "Started port forwarding for container %s" (substring container-id 0 12)))))

(defun devcontainer-stop-port-forwarding ()
  "Stop port forwarding for the current devcontainer."
  (interactive)
  (when-let* ((project (project-current))
              (proc (alist-get project devcontainer--port-forwarder-alist nil nil 'equal)))
    (when (process-live-p proc)
      (kill-process proc)
      (message "Stopped port forwarding"))
    (setq devcontainer--port-forwarder-alist
          (assoc-delete-all project devcontainer--port-forwarder-alist 'equal))
    (when (eq proc devcontainer--port-forwarder-process)
      (setq devcontainer--port-forwarder-process nil))))

(defun devcontainer--maybe-start-port-forwarding (container-id _container-name _remote-user _remote-workdir)
  "Start port forwarding if enabled.

This function is designed to be added to `devcontainer-post-startup-hook'.
CONTAINER-ID is used to identify the container.  Other arguments are ignored."
  (when (and devcontainer-enable-port-forwarding
             container-id
             (devcontainer--port-forwarder-script))
    (devcontainer-start-port-forwarding)))

;; NOTE: Port forwarding hook is NOT automatically added.
;; To enable automatic port forwarding, add this to your init.el:
